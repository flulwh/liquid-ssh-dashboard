    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4 px-0.5">
        <div>
          ...
          <p className="mt-1 text-sm text-white/45">{t('dashboard.subtitle')}</p>
        </div>
        <div className="flex items-center gap-2">
          <button
          ...
          <Link to="/servers" className="glass-btn !bg-white/10 font-medium text-white">
            <Plus className="h-4 w-4" /> {t('dashboard.manageServers')}
          </Link>
        </div>
      </div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
            <StatCard
              ...
              value={String(online)}
              sub={t('dashboard.statOnlineSub', { n: servers.length })}
              accent="emerald"
              delay={0}
            />
            <StatCard
              ...
              sub={reach.refreshing ? t('dashboard.statOfflineChecking') : t('dashboard.statOfflineResult')}
              accent="rose"
              delay={0.05}
            />
            <StatCard
              label={t('dashboard.statAvgCpu')}
              value={avgCpu ? `${avgCpu}%` : '—'}
              sub={t('dashboard.statAvgCpuSub')}
              accent="violet"
              delay={0.1}
            />
            <StatCard
              label={t('dashboard.statBackend')}
              value={reach.backendOk ? t('common.online') : t('common.offline')}
              sub={getApiBase() || t('dashboard.statBackendUnconfigured')}
              accent="sky"
              delay={0.15}
            />
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            <GlassCard spotlight={false} className="overflow-hidden lg:col-span-2">
              <div className="flex items-center justify-between border-b border-white/[0.06] px-5 py-4">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-sm font-semibold text-white">{t('dashboard.serversTitle')}</h2>
                ...
                <div className="glass ml-auto flex w-56 items-center gap-2 rounded-lg px-2.5 py-1.5">
                  ...
                  <input
                    ...
                    placeholder={t('dashboard.serversFilter')}
                ...
                {filtered.length === 0 ? (
                  <div className="flex flex-col items-center gap-3 py-16 text-center">
                    ...
                    <p className="text-sm text-white/45">
                      {servers.length === 0 ? t('dashboard.empty') : t('dashboard.emptyNoMatch')}
                    </p>
                    <Link to="/servers" className="glass-btn">
                      <Plus className="h-4 w-4" /> {t('dashboard.addServer')}
                    </Link>
                  </div>
                ) : (
                  filtered.map((s, i) => (
            ...
            <GlassCard spotlight={false} className="flex flex-col p-5">
              <h3 className="text-sm font-semibold text-white">{t('dashboard.endpoints')}</h3>
              <div className="mt-4 space-y-3">
                <Endpoint
                  label={t('dashboard.endpointApi')}
                  value={getApiBase() || t('dashboard.endpointUnconfigured')}
                  state={reach.backendOk ? 'ok' : 'down'}
                />
                <Endpoint
                  label={t('dashboard.endpointSsh')}
                  value={`${(getApiBase() || '').replace(/^http/, 'ws')}/ws/ssh` || '—'}
                  state={reach.backendOk ? 'ok' : 'down'}
                />
              </div>
              <div className="mt-auto pt-5 text-xs text-white/35">{t('dashboard.endpointNote')}</div>
            </GlassCard>
          </div>
        </>
      )}
        ...
        <div className="flex items-center gap-3">
          ...
          {load && (
            <div className="hidden min-w-0 flex-1 grid-cols-3 gap-3 sm:grid">
              <MetricBar label={t('dashboard.cpu')} value={Math.round(load.cpu)} />
              <MetricBar label={t('dashboard.mem')} value={load.mem.percent} />
              <MetricBar label={t('dashboard.disk')} value={load.disk.percent} />
            </div>
          )}
        </div>
        {load ? (
          ...
          <div className="mt-1 text-xs text-white/30">{t('dashboard.noCreds')}</div>
        ) : null}
      </div>
      <span className="hidden font-mono text-xs text-white/40 sm:block">
        {checking ? t('dashboard.probing') : reachable ? `${latency}ms` : t('dashboard.unreachable')}
      </span>
      <button
        onClick={onOpen}
